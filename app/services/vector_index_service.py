...
class VectorIndexService:
    ...
    def index_directory(self, directory_path: Optional[str] = None) -> IndexingResult:
        ...
        try:
            ...
            # 获取所有支持的文件（与 document_loader.py 中注册的类型保持同步）
            files = (
    ...
    def index_single_file(self, file_path: str):
        ...
        try:
            # 1. 读取文件内容（使用文档加载器工厂，自动适配 PDF/Word/MD/TXT）
            from app.services.document_loader import document_loader_factory

            content = document_loader_factory.load(path)
