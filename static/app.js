...
class SuperBizAgentApp {
    constructor() {
        // Use a relative URL so the page works on localhost, a LAN address, or behind a proxy.
        this.apiBaseUrl = '/api';
    ...
    renderMarkdown(content) {
        ...
        try {
            // marked turns Markdown into HTML, but it is not a sanitizer. AI output and
            // uploaded documents are untrusted, so only allow a narrow set of safe tags.
    ...
    // Keep Markdown formatting while removing scripts, event handlers, unsafe links,
    // images, and other browser-executable HTML.
    sanitizeHtml(html) {
        ...
        const allowedTags = new Set([
            ...
            'H5', 'H6', 'HR', 'LI', 'OL', 'P', 'PRE', 'S', 'STRONG', 'TABLE', 'TBODY',
            'TD', 'TH', 'THEAD', 'TR', 'UL'
        ]);
        const removeWithContent = new Set([
            'BASE', 'EMBED', 'FORM', 'FRAME', 'IFRAME', 'LINK', 'META', 'OBJECT',
        ...
        [...template.content.querySelectorAll('*')].forEach((element) => {
            ...
            [...element.attributes].forEach((attribute) => {
                ...
                const keepListStart = element.tagName === 'OL' && name === 'start' && /^\d+$/.test(value);
                const keepHref = element.tagName === 'A' && name === 'href' && this.isSafeLink(value);

                if (!keepCodeLanguage && !keepListStart && !keepHref) {
    ...
    initializeElements() {
        ...
        this.welcomeAIOpsBtn = document.getElementById('welcomeAIOpsBtn');
    ...
    bindEvents() {
        ...
        if (this.knowledgeUploadBtn) {
            this.knowledgeUploadBtn.addEventListener('click', () => this.fileInput?.click());
        }
        ...
        if (this.welcomeAIOpsBtn) {
            this.welcomeAIOpsBtn.addEventListener('click', () => this.triggerAIOps());
        }
        ...
        document.querySelectorAll('#quickPrompts button[data-prompt]').forEach((button) => {
    ...
    async checkServiceStatus() {
        ...
        try {
            ...
            const isHealthy = payload?.data?.status === 'healthy';

            this.serviceStatus.classList.toggle('is-ready', isHealthy);
            this.serviceStatus.classList.toggle('is-error', !isHealthy);
            this.serviceStatus.lastChild.textContent = isHealthy ? '服务运行正常' : '服务需要检查';
        } catch (_) {
            this.serviceStatus.classList.remove('is-ready');
            this.serviceStatus.classList.add('is-error');
            this.serviceStatus.lastChild.textContent = '服务暂不可用';
        }
    }
    ...
    renderChatHistory() {
        ...
        if (this.historyCount) {
            this.historyCount.textContent = this.chatHistories.length
                ? `已保存 ${this.chatHistories.length} 条`
                : '暂无记录';
        }
    ...
    toggleModeDropdown() {
        if (this.modeSelectorBtn && this.modeDropdown) {
            ...
            if (wrapper) {
                ...
                this.modeSelectorBtn.setAttribute('aria-expanded', String(isOpen));
            }
        }
    }
    ...
    updateUI() {
        ...
        if (this.currentModeText) {
            ...
            this.currentModeText.textContent = modeNames[this.currentMode] || '快速回答';
        }
                                ...
                                if (sseMessage && typeof sseMessage.type === 'string') {
                                    if (sseMessage.type === 'content') {
                                        ...
                                        if (toolData.status === 'start') {
                                            this.showToolIndicator(assistantMessageElement, toolData.tool, 'running', toolData.input);
                                        } else if (toolData.status === 'end') {
                                            this.showToolIndicator(assistantMessageElement, toolData.tool, 'done');
                                        }
                                    } else if (sseMessage.type === 'search_results') {
                                        const resultData = sseMessage.data || {};
                                        this.showSearchResultsIndicator(assistantMessageElement, resultData);
                                    } else if (sseMessage.type === 'done') {
                                        this.finalizeToolIndicators(assistantMessageElement);
                                        this.handleStreamComplete(assistantMessageElement, fullResponse);
                                        return;
                                    } else if (sseMessage.type === 'error') {
                                        console.error('[SSE] 收到错误:', sseMessage.data);
                                        if (assistantMessageElement) {
                                            const messageContent = assistantMessageElement.querySelector('.message-content');
    ...
    static TOOL_LABELS = {
        ...
        'query_memory_metrics': '💾 查询内存指标',
        'query_disk_metrics': '💿 查询磁盘指标',
        'query_process_info': '🔍 查询进程信息',
        'search_log': '📋 查询日志',
        'get_current_timestamp': '⏱️ 获取时间戳',
        'search_topic_by_service_name': '🔎 搜索日志主题',
        'get_topic_info_by_name': '📑 获取主题信息',
        'get_region_code_by_name': '🌐 获取地区信息',
    };

    showToolIndicator(messageElement, toolName, status, input = null) {
        if (!messageElement) return;

        let indicator = messageElement.querySelector('.tool-indicators');
        if (!indicator) {
            indicator = document.createElement('div');
            indicator.className = 'tool-indicators';
            // 插入到消息内容之前
            const content = messageElement.querySelector('.message-content');
            if (content) {
                messageElement.insertBefore(indicator, content);
            } else {
        ...
        if (status === 'running') {
            const label = SuperBizAgentApp.TOOL_LABELS[toolName] || `🛠️ 调用 ${toolName}`;
            // 移除同名的旧指示器
            const existing = indicator.querySelector(`[data-tool="${toolName}"]`);
            if (existing) existing.remove();

            const item = document.createElement('span');
            item.className = 'tool-indicator tool-running';
            item.setAttribute('data-tool', toolName);
            item.innerHTML = `<span class="tool-spinner"></span> ${label}...`;
            indicator.appendChild(item);

            // 3 秒后自动标记为完成（防止卡住）
            clearTimeout(this[`_toolTimeout_${toolName}`]);
            this[`_toolTimeout_${toolName}`] = setTimeout(() => {
                this.showToolIndicator(messageElement, toolName, 'done');
            }, 8000);
        } else if (status === 'done') {
            const label = SuperBizAgentApp.TOOL_LABELS[toolName] || toolName;
            const existing = indicator.querySelector(`[data-tool="${toolName}"]`);
            if (existing) {
                existing.className = 'tool-indicator tool-done';
                existing.innerHTML = `✅ ${label} 完成`;
                // 3 秒后渐变消失
                setTimeout(() => {
    ...
    showSearchResultsIndicator(messageElement, resultData) {
        if (!messageElement) return;
        const indicator = messageElement.querySelector('.tool-indicators');
        if (!indicator) return;

        const item = document.createElement('span');
        item.className = 'tool-indicator tool-result';
        const count = resultData.count || 0;
        const sources = (resultData.sources || []).join(', ');
        item.innerHTML = `📄 找到 ${count} 条相关内容` + (sources ? ` (${sources})` : '');
        indicator.appendChild(item);
        setTimeout(() => {
    ...
    finalizeToolIndicators(messageElement) {
        if (!messageElement) return;
        const indicator = messageElement.querySelector('.tool-indicators');
        if (indicator) {
            // 标记所有 running 状态的为 done
            const running = indicator.querySelectorAll('.tool-running');
            running.forEach(el => {
                const toolName = el.getAttribute('data-tool');
                el.className = 'tool-indicator tool-done';
                el.innerHTML = `✅ ${el.textContent.replace(/\.+$/, '')} 完成`;
            });
            // 1 秒后全部移除
            setTimeout(() => {
    ...
    handleFileSelect(event) {
        ...
        if (file) {
            ...
            if (!this.validateFileType(file)) {
                this.showNotification('只支持 TXT / Markdown / PDF / Word 格式的文件', 'error');
    ...
    async uploadFile(file) {
        ...
        // Keep this limit aligned with the server so users get a useful message
        // before a potentially slow upload begins.
        const maxSize = 20 * 1024 * 1024;
        if (file.size > maxSize) {
            this.showNotification('文件大小不能超过 20MB', 'error');
            return;
        }
        ...
        try {
            ...
            if ((data.code === 200 || data.message === 'success') && data.data) {
                ...
                const successMessage = indexed
                    ? `“${file.name}” 已导入知识库，可以开始提问了。`
                    : `“${file.name}” 已保存，但暂未完成索引，暂时不能用于检索。请稍后重试或查看服务日志。`;
                this.addMessage('assistant', successMessage, false, true);
                if (!indexed) this.showNotification('文档已保存，但索引失败', 'warning');
            } else {
